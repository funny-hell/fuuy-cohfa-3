...
const vec4 light_color = vec4(-0.8, -0.8, -1, 0.5); //r, g, b, a. the gradient light that is above (and on top) of the water
const vec4 glow_color = vec4(0, 0, 0, .5); //alpha is unused here. the color that white parts glow when near water
...
void frag(){
    ...
    dy += noise(worldPos.y / 16.0 +timer  * -4.35)*3;
    ...
    glowval3 = glowval3*light_distance / (playerglow_strength*0);
    glowval3 += 1;
    glowval3 = clamp(glowval3, 0.0, 1.0);
    glowval3 = glowval3*glowval3 * playerglow_strength*0;
