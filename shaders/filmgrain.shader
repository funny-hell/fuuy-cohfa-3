...
const float resolution = 20.0;                   // the resolution of the selected colors (lower = more limited)
const float saturation = 1.2;                    // how saturated the resulting colors should be
const vec2  mosaic_size = vec2(450.0, 450.0);    // the smaller the number the larger the on-screen pixels
...
void frag()
{
    ...
    dy += noise(world_pos.x / 32.0 + timer *  2.00) * 1.0;
    dy += noise(world_pos.x / 16.0 + timer * -4.35) * 0.5;
    dy += noise(world_pos.x /  8.0 + timer *  1.00) * 0.25;
    ...
    float pixelyness = 4;
    float amplitude = 1;
    vec2 coords = vec2(floor(gl_FragCoord.x/pixelyness)*pixelyness,floor(gl_FragCoord.y/pixelyness)*pixelyness);

    outcolor *= 1 + fract(sin(dot(coords, vec2(12.9898+mod(timer+1,3)*85, 78.233+mod(timer+1,3)*13))) * 43758.5453)/amplitude;
