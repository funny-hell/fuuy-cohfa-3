...
const vec4  light_color = vec4(1.0, 0.85, 0.20, 0.4); // r, g, b, a. the gradient light that is above (and on top) of the water
const float light_distance = 6;                    // less = farther distance
const vec4  glow_color = vec4(1.0, 0.85, 0.8, 0.3);  // alpha is unused here. the color that white parts glow when near water
const float glow_strength = 12;                      // how strong the glow effect is
const float glow_threshold = 0.25;                   // how bright the green channel needs to be to trigger a glow
