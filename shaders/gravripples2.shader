...
varying vec2 worldPos;
    ...
    void frag(){
        ...
        vec2 rippleval = vec2(0.0, sin(timer*1+worldPos.y * .01)*cos(timer*1+worldPos.x * .01)* .01);
        
        vec4 outcolor = texture(framebuf, screencoords.xy + rippleval * ratio);
        outcolor = sin(timer*1+worldPos.y * .01)*cos(timer*1+worldPos.x * .01)*sin(timer*1)*1.2*outcolor*(1.0-gl_Color.a) + gl_Color*gl_Color.a;
