...
const vec4  light_color = vec4(1, 1, 1, 1.0); // r, g, b, a. the gradient light that is above (and on top) of the water
const float light_distance = 0.2;                    // less = farther distance
const vec4  glow_color = vec4(0.5, 0.5, 0.5, 1);  // alpha is unused here. the color that white parts glow when near water
...
void frag()
{
    ...
    dy -= noise(worldPos.x / 32.0/5 + timer *  2   ) * 4.5;
    dy -= noise(worldPos.x / 16.0/5 + timer * -4.35) * 9;
    //dy -= noise(worldPos.x / 8.0/5  + timer *  1   ) * 18;
    dy -= noise(worldPos.x / 72.0/5  + timer *  -3   ) * 36;
    dy -= noise(worldPos.x / 144.0/5  + timer *  3   ) * 72;
