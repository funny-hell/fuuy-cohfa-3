...
vec4  light_color = vec4(0.5, 0.45, 0.2, 1.0); // r, g, b, a. the gradient light that is above (and on top) of the water
...
const float glow_strength = 5;                 //how strong the glow effect is
...
void frag()
{
    ...
    vec2 rippleval = vec2(sin(-timer * 2.2 + sin(timer / 3 + worldPos.y / 7 + worldPos.y / 11 + worldPos.y / 4) / 6 + worldPos.x / 15.0) * 0.004 * ratio.x, sin(-timer * 3 + sin(timer / 2 + worldPos.x / 7 + worldPos.x / 13 + worldPos.x / 4) / 6 + worldPos.y / 15.0) * 0.004 * ratio.y);
    ...
    float dy = waterheight - worldPos.y - 10;

    dy += noise(worldPos.x / 32.0 + timer *  2   ) * 1.5;
    dy += noise(worldPos.x / 16.0 + timer * -4.35) * 3;
    dy += noise(worldPos.x / 8.0  + timer *  1   ) * 6;
...
float deviance = lightval; //(noise(worldPos.x/32.0-timer*2)+noise(noise(worldPos.y/32)-timer))/3;
float xsplit = worldPos.y/1080+worldPos.x/1920+timer+deviance;
const float mul = 4;
const float bright = 0.2;
light_color = vec4(sin(mul*xsplit)+bright, sin(mul*xsplit+2.09439510239)+bright, sin(mul*xsplit+4.18879020479)+bright, 1) - vec4(0,0,0,0.7); // r, g, b, a. the gradient light that is above (and on top) of the water
