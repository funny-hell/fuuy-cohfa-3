...
varying vec2 worldPos;
    ...
    void frag(){
        ...
        vec2 rippleval = vec2(sin(timer*2.0+worldPos.y / 20.0) * .005, sin(timer*3.0+worldPos.y / 10.0) * .002) * ratio;
        
        vec4 outcolora = texture2D(framebuf, screencoords.xy);
        vec4 outcolorb = texture2D(framebuf, screencoords.xy + rippleval);
